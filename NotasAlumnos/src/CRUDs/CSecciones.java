...
/*
    ...
    public static boolean actualizarSeccion(int seccionId, String nuevoNombreSeccion) {
        boolean flag = false;
        Session session = HibernateUtil.HibernateUtil.getSessionFactory().openSession();
        Transaction transaction = null;
        ...
        try {
            ...
            if (seccion != null) {
                seccion.setNombreSeccion(nuevoNombreSeccion);  // Actualizar el nombre de la sección

                // Actualizar la sección en la base de datos
                session.update(seccion);
                flag = true;
            } else {
                System.out.println("No se encontró la sección con ID " + seccionId);
            }
    ...
    public static boolean eliminarSeccion(int seccionId) {
        boolean flag = false;
        Session session = HibernateUtil.HibernateUtil.getSessionFactory().openSession();
        Transaction transaction = null;
        ...
        try {
            ...
            Secciones seccion = (Secciones) session.get(Secciones.class, seccionId);
            if (seccion != null) {
                session.delete(seccion);  // Eliminar la sección
                flag = true;
            } else {
                System.out.println("No se encontró la sección con ID " + seccionId);
            }
